	...
	namespace Tracer {
			...
			float3 tx, float3 rx, ExactPath* out_paths)
		{
			...
			for (int i = 0; i < MAX_BOUNCE_DEPTH; ++i) {
				...
				path.hit_materials[i] = 255; // [新增] 255 代表无效材质
			}
			...
			for (int i = 0; i < numBounces; ++i) {
				...
				path.hit_materials[i] = topo.nodes[i].material_id; // [新增] 提取材质 ID
			}
			...
			for (int i = 0; i < numBounces; ++i) {
				...
				path.hit_objects[i] = topo.nodes[i].plane_label;
				path.hit_materials[i] = topo.nodes[i].material_id; // [新增] 再次确保写入准确的材质 ID
			}
